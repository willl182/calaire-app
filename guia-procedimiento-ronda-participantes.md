...
| **`u_xi`** | Incertidumbre estándar del valor medio reportado `xi`. En pantalla aparece como `Incertidumbre estándar u(x)`. |
| **`U_xi`** | Incertidumbre expandida del valor medio reportado `xi`. En pantalla aparece como `Incertidumbre expandida U(X)`. |
| **Factor de cobertura `k`** | Relación `k = U_xi / u_xi`, siempre que `u_xi > 0`. En pantalla aparece como `Factor de cobertura k`. Debe poder recuperarse para reportes. |
...
En la pantalla del aplicativo este valor se carga en el campo `Incertidumbre estándar u(x)`.
...
En la pantalla del aplicativo este valor se carga en el campo `Incertidumbre expandida U(X)`.
...
- Que `Incertidumbre expandida U(X)` sea coherente con `Incertidumbre estándar u(x)` y el factor de cobertura usado.
...
| `Incertidumbre estándar u(x)` | Participante | Incertidumbre estándar de `xi`, `u_xi`. |
| `Factor de cobertura k` | Participante | Factor de cobertura usado para obtener la incertidumbre expandida. |
| `Incertidumbre expandida U(X)` | Participante | Incertidumbre expandida de `xi`, `U_xi`. |
...
- El aplicativo no recalcula ningún campo numérico (`Promedio`, `Desv. Est.`, `Incertidumbre estándar u(x)`, `Factor de cobertura k`, `Incertidumbre expandida U(X)`). Todos los valores numéricos son ingresados directamente por el participante.
...
|------------------|-----------|
| `scripts/preprocesar_calaire.R` | Ejecuta el pipeline principal de preprocesamiento de Calaire para una ronda. |
| `scripts/run_preprocessor_calaire.R` | Procesa múltiples archivos de ronda detectados en `data/raw/`. Útil para ejecución por lotes. |
| `scripts/unir_rondas.R` | Une salidas procesadas de participantes y referencia para generar archivos consolidados usados por el aplicativo y el análisis estadístico. |
...
`scripts/unir_rondas.R` toma los archivos procesados de participantes y referencia, asigna o propaga campos internos como `tipo` (`participante` o `referencia`) y `n_lab` (número de laboratorios), y genera el archivo consolidado.
...
| `Incertidumbre estándar u(x)` | `u_xi` | Incertidumbre estándar de `xi`. |
| `Factor de cobertura k` | `k` | Factor de cobertura usado para estimar la incertidumbre expandida. |
| `Incertidumbre expandida U(X)` | `U_xi` | Incertidumbre expandida de `xi`. |
...
| `u_value` | Incertidumbre estándar `u_xi` | Participante (como `Incertidumbre estándar u(x)`) |
...
- [ ] He estimado `Incertidumbre estándar u(x)` (`u_xi`) de acuerdo con mi propio criterio metrológico.
- [ ] He definido el `Factor de cobertura k` usado en mi reporte.
- [ ] He estimado `Incertidumbre expandida U(X)` (`U_xi`) de acuerdo con mi propio criterio metrológico.
- [ ] He verificado que `Incertidumbre expandida U(X)` sea coherente con `Incertidumbre estándar u(x)` y mi factor de cobertura.
...
├─ Incertidumbre estándar u(x) ──>  u_value      ──>  u_xi (incertidumbre estándar)
├─ Factor de cobertura k       ──>  [futuro k]   ──>  k (factor de cobertura)
├─ Incertidumbre expandida U(X) ──>  [futuro U_xi] ──>  U_xi (incertidumbre expandida)
│
...
1. `scripts/preprocesar_calaire.R`
2. `scripts/run_preprocessor_calaire.R`
3. `scripts/unir_rondas.R`
