...
Fusión detallada de `Fab_part_plan.md` (mecánica técnica, fases ejecutables) y `sol_part_plan.md` (reglas de negocio, ciclo de mejora). Análisis comparativo: `s_rev_p.md`. Workflow: `s_final_w.md`. Aceptación: `s_final_t.md`.
...
| Publicación | Import crea una evaluación borrador reimportable. Una mutation corta cambia su cabecera a `publicada`, haciendo visibles de forma atómica todos sus puntajes e informe, solo en `documentacion_pendiente`/`cerrada`. Certificados opcionales, notificaciones y creación de casos se procesan después de forma idempotente. **Sin edición, reemplazo ni despublicación posterior**: la doble verificación ocurre antes, en pt_app |
...
En las acciones de registro bajo `app/(protected)/ronda/[codigo]/registro/`: cambiar `estado === 'cerrada'` por `estado !== 'activa'`. Replicar server-side en `convex/rondas/fichas.ts` (hoy no validan estado). Confirmar nombres y líneas exactas al implementar.
...
### 3.1 Contrato CSV (validado)

Contrato confirmado con `_workspace/Puntajes_Finales_PT_2026-07-12.csv` (UTF-8, encabezados entre comillas, decimal con punto). Documentar en `_workspace/feats/pt_scores_csv.md`:
...
Claves de join: `participant_code` contra `rondaParticipantes.participantCode`; (`contaminante`,`run_code`,`level_label`) contra `rondaPtItems`. `metodo` distingue resultados dentro del mismo ítem: la muestra contiene dos métodos por ítem, por lo que la clave única es participante × ítem × método.

Reglas de datos:

- `clasificacion` acepta exactamente los valores observados `Satisfactorio` y `No satisfactorio`, normalizados al importar a `satisfactorio` y `no_satisfactorio`. Es la decisión oficial importada; Calaire no reclasifica.
- Valores estadísticos que no apliquen: campo vacío → `null`, nunca cero. Rechazar no finitos (`NaN`, `Inf`).
- Sin duplicados participante × ítem × método; códigos desconocidos o filas no asociables bloquean.

La muestra tiene 20 filas, un participante, 12 resultados satisfactorios y 8 no satisfactorios, sin vacíos ni duplicados de la clave completa. Todos los valores de evaluación se importan. `n` y bins de histograma no vienen como columnas: si se muestran, se derivan exclusivamente de las filas importadas como agregados de visualización; no recalculan puntajes ni clasificación. Se eliminan `media` y `sd` del agregado porque no son necesarios ni están en el contrato.
...
ptScores: defineTable({
  ...
  .index('by_rondaId_and_estadoPublicacion', ['rondaId', 'estadoPublicacion'])
...
}).index('by_rondaId', ['rondaId']),         // mutations consultan con .unique(); la app impide una segunda cabecera
...
- `validateScoreRows(rows, ronda, participantes, items)` — función pura compartida CSV/HTTP. Devuelve `{ filasValidas, errores: [{fila, campo, mensaje}] }`. Reglas: participante existente y reclamado, ítem existente, clasificación exacta y normalizable, números parseables/finitos, vacío→null, unidad y método no vacíos, sin duplicados participante × ítem × método.
- `previewImport` (query admin): validación sin escribir.
- `importScoresDraft` (mutation admin, `requireAdminIdentity` — mismo helper que `createPTItem`): reemplaza el borrador de la ronda en lotes idempotentes, inserta filas como `borrador`, deriva solamente `n`/bins de visualización y marca `ptEvaluaciones` como `borrador_validado` al completar. Nunca toca una evaluación publicada. Audit vía `writeAudit`.
- `publicarResultados` (mutation admin): **precondiciones** — ronda en `documentacion_pendiente` o `cerrada`, evaluación en `borrador_validado`, import completo e informe general cargado. La mutation no recorre todas las filas: cambia únicamente la cabecera a `publicada` y sella `publicadaAt/By`. Todas las queries condicionan la lectura a esa cabecera, por lo que resultados, stats e informe aparecen juntos o no aparecen. Después agenda trabajos idempotentes para casos, notificaciones y certificados opcionales; esos efectos no forman parte de la atomicidad de visibilidad.
...
- Parseo CSV en un módulo server-only bajo `lib/` junto al adaptador de rondas existente, con tests unitarios de encoding/decimales/columnas faltantes. No crear un árbol paralelo `src/`.
...
UI participante — ruta bajo `app/(protected)/ronda/[codigo]/resultados/page.tsx` (server component):
...
UI admin (panel de ronda bajo `app/(protected)/dashboard/rondas/[id]/resultados/`):
...
### 3.5 Certificados (opcionales, posteriores a la publicación)

- Implementar con una solución PDF server-side compatible con el proyecto; la elección concreta de librería es una decisión técnica de implementación, no un bloqueo funcional.
- Schema `ptCertificados` (rondaId, rondaParticipanteId, storageId, createdAt/By).
- Generados idempotentemente después de publicar para cada participante con envío final. Un fallo no revierte ni oculta resultados; se registra como `pendiente`/`fallido` y permite reintento. Disponibles aunque exista caso abierto.
...
- UI en `app/(protected)/mi-dashboard` (confirmar con usuario; alternativa conservadora: dentro de resultados de ronda, según restricción de Sol):
...
Después de publicar, si el participante tiene ≥1 resultado **no satisfactorio**: crear idempotentemente **un solo caso** por participante/ronda (sobre `sgcCasos`, tipo `nc_capa`) agrupando todos esos resultados.
...
Cierre exige conjuntamente: (1) aceptación documental por Calaire y (2) un resultado satisfactorio posterior para **cada** resultado que originó el caso.

- Sin participación posterior: el caso queda abierto indefinidamente; el calendario muestra la próxima oportunidad.
- Correspondencia predeterminada y comprensible: mismo participante, contaminante, nivel y método en una ronda posterior. Si cambian los identificadores técnicos entre rondas, un administrador puede seleccionar manualmente cuál resultado posterior corresponde, dejando la decisión en bitácora.
- Verificación parcial no cierra; se listan pendientes. Otro resultado no satisfactorio exige una nueva iteración documental con historial.
...
casoResultadosOrigen: defineTable({          // scores no satisfactorios agrupados en el caso
  casoId: v.id('sgcCasos'),
...
- En `/resultados`: fila no satisfactoria enlaza al caso automático.
...
- **Export ICS**: route handler `app/(protected)/calendario/ics/route.ts` — formato iCalendar generado a mano (texto plano, VEVENT por hito); validar importándolo en Google Calendar.
...
| Publicación | convex-test: precondiciones de estado de ronda; cambio atómico de cabecera; import incompleto no publicable; participante no ve borradores; ve publicados propios; nunca filas ajenas; jobs posteriores idempotentes |
...
1. **Contrato CSV resuelto:** usar `_workspace/Puntajes_Finales_PT_2026-07-12.csv` como fixture inicial y documentar su contrato. Añadir fixtures futuros si aparecen campos vacíos u otros participantes.
2. Todos los puntajes, incertidumbres, estadísticos y clasificaciones se importan. Solo `n` y bins se derivan para visualización; no se calcula desempeño.
3. Certificado opcional: elegir la implementación PDF durante la fase 1; su generación no bloquea la publicación ni requiere rollback de resultados.
4. **Estructura del repositorio:** conservar el layout indicado por `AGENTS.md` (`app/`, `lib/`, `app/components/`, `convex/`) y confirmar cada ubicación real antes de implementar. No introducir `src/`.
...
6. **Verificación de eficacia entre rondas:** emparejar automáticamente por participante + contaminante + nivel + método; permitir selección admin auditada cuando esos identificadores cambien.
...
Fase 0 → documentar y convertir el CSV real en fixture → Fase 1 (schema + import + publicación por cabecera + resultados + certificado opcional + descargas) → Fase 1b HTTP → Fase 2 dashboard → Fase 3 casos + verificación → Fase 4 calendario.
