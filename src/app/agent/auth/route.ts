...
import { createAgentClaim } from '@/server/auth/agent-auth'
import { sendAgentClaimEmail } from '@/server/mailer'
...
export async function POST(req: Request) {
  ...
  if (body?.type !== 'identity_assertion' || body?.assertion_type !== 'verified_email') {
    return NextResponse.json({ error: 'unsupported_flow' }, { status: 422 })
  }
