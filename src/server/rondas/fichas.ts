...
export async function getOrCreateFicha(rondaParticipanteId: string): Promise<FichaRegistro> {
  const id = await fetchMutation(api.fichas.index.getOrCreateFicha, {
    rondaParticipanteId: rondaParticipanteId as Id<'rondaParticipantes'>,
  })
  const doc = await fetchQuery(api.fichas.index.getFichaById, {
...
export async function upsertFichaScalars(
  ...
  await fetchMutation(api.fichas.index.upsertFichaScalar, {
...
export async function adminUpsertFichaScalars(
  ...
  const convexField = SCALAR_FIELD_MAP[field]
  await fetchMutation(api.fichas.index.adminUpsertFichaScalar, {
    fichaId: fichaId as Id<'fichasRegistro'>,
    field: convexField,
    ...(typeof value === 'boolean'
      ? { valueBoolean: value }
...
export async function replaceAcompanantes(
  ...
  await fetchMutation(api.fichas.index.replaceAcompanantes, {
    fichaId: fichaId as Id<'fichasRegistro'>,
...
export async function adminReplaceAcompanantes(
  ...
  await fetchMutation(api.fichas.index.adminReplaceAcompanantes, {
    fichaId: fichaId as Id<'fichasRegistro'>,
    items: items.map((item) => ({
      sortOrder:          item.sort_order,
      nombreCompleto:     item.nombre_completo,
...
export async function generateFichaUploadUrl(): Promise<string> {
  return fetchMutation(api.fichas.index.generateFichaUploadUrl, {})
}
...
export async function replaceAnalizadores(
  ...
  await fetchMutation(api.fichas.index.replaceAnalizadores, {
    fichaId: fichaId as Id<'fichasRegistro'>,
    items: items.map((item) => ({
      sortOrder:              item.sort_order,
...
export async function adminReplaceAnalizadores(
  ...
  await fetchMutation(api.fichas.index.adminReplaceAnalizadores, {
    fichaId: fichaId as Id<'fichasRegistro'>,
    items: items.map((item) => ({
      sortOrder:              item.sort_order,
      analito:                item.analito,
...
export async function replaceInstrumentos(
  ...
  await fetchMutation(api.fichas.index.replaceInstrumentos, {
    fichaId: fichaId as Id<'fichasRegistro'>,
    items: items.map((item) => ({
      sortOrder:   item.sort_order,
      equipo:      item.equipo,
...
export async function adminReplaceInstrumentos(
  ...
  await fetchMutation(api.fichas.index.adminReplaceInstrumentos, {
    fichaId: fichaId as Id<'fichasRegistro'>,
    items: items.map((item) => ({
      sortOrder:   item.sort_order,
      equipo:      item.equipo,
...
export async function submitFicha(fichaId: string): Promise<void> {
  await fetchMutation(api.fichas.index.submitFicha, {
    fichaId: fichaId as Id<'fichasRegistro'>,
  })
}
